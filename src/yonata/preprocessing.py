...
def image_file_path_to_bytes(image_file_path):
...
def image_numpy_to_bytes(image_numpy):
...
def image_to_bytes(image_input, maximum_bytes_size=MB_8):
    # Handle both file path and numpy array inputs
    if isinstance(image_input, str):
        image_bytes = image_file_path_to_bytes(image_input)
    elif 'numpy' in str(type(image_input)):
        image_bytes = image_numpy_to_bytes(image_input)
    else:
        raise TypeError(f"Unsupported input type: {type(image_input)}. Expected str path or numpy array.")

    # Check if image needs resizing
    if len(image_bytes) > maximum_bytes_size:
        print(f"Image size ({len(image_bytes)/1024/1024:.2f}MB) exceeds limit. Resizing...")
